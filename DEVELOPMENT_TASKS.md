...
- [x] 实现互补滤波。
- [x] 实现一维卡尔曼滤波。
- [x] 构造位置/角度真实值、慢漂移测量、高频噪声测量。
- [x] 显示真实值、传感器 A、传感器 B、融合结果。
- [x] 参数支持互补系数、过程噪声 Q、测量噪声 R。
- [x] 补充卡尔曼滤波的直观解释，避免只堆公式。
...
- [x] 实现 `RobotArmWidget`，绘制 2D 二连杆或三连杆机械臂。
- [x] 将关节角信号映射到机械臂姿态。
- [x] 显示滤波前末端轨迹与滤波后末端轨迹。
- [x] 提供关节抖动、指令突变、周期振动等预设。
- [x] 增加末端轨迹拖尾显示。
...
- [x] 构造力/力矩模拟信号。
- [x] 支持接触突变、短时冲击、随机尖峰。
- [x] 对比滑动平均、EWMA、中值、Hampel。
- [x] 添加“响应速度 vs 平滑程度”的解释。
...
完成内容：新增 NotchFilter 和 BandPassFilter，使用二阶 biquad 结构实现陷波和带通；主窗口为陷波接入中心频率、带宽参数，为带通接入低截止和高截止参数；说明区补充陷波/带通的用途、公式思路、参数影响、优缺点和简化频率响应示意；沿用阶段 2 的周期噪声生成能力用于周期干扰演示。

测试结果：Release 构建通过，生成 build/release/Filtering.exe；Debug 构建通过，生成 build-debug/debug/Filtering.exe。构建过程中仍出现 Qt license client 锁文件警告，但最终退出码为 0。

遗留问题：尚未打开 GUI 人工观察陷波是否对准并压制周期干扰、带通是否保留目标频段；可选带阻滤波器未实现。

下一步：进入阶段 5，实现互补滤波和一维卡尔曼滤波，搭建传感器融合演示数据流。
```
...
完成内容：新增 BandStopFilter，使用二阶 biquad 带阻结构；主窗口新增“带阻滤波器”条目，复用低截止/高截止参数控件；说明区补充带阻用途、参数影响、优缺点和简化频率响应示意。

测试结果：Release 构建通过，生成 build/release/Filtering.exe；Debug 构建通过，生成 build-debug/debug/Filtering.exe。

遗留问题：仍需打开 GUI 人工观察带阻是否正确抑制指定阻带。

下一步：完成构建验证后继续阶段 5 传感器融合演示。
```
...
完成内容：扩展 SignalFrame 和 SignalChartWidget，支持显示真实值、传感器 A、传感器 B、融合结果四条曲线；新增 ComplementaryFilter 和 KalmanFilter1D；互补滤波器使用传感器 B 的快速变化预测并用传感器 A 修正低频漂移；卡尔曼滤波器支持过程噪声 Q 和测量噪声 R；主窗口为互补滤波接入 alpha 参数，为卡尔曼滤波接入 Q/R 参数；新增融合场景数据流，模拟慢漂移测量和高频噪声测量。

测试结果：Release 构建通过，生成 build/release/Filtering.exe；Debug 构建通过，生成 build-debug/debug/Filtering.exe。构建过程中仍出现 Qt license client 锁文件警告，但最终退出码为 0。

遗留问题：尚未打开 GUI 人工观察互补滤波与卡尔曼滤波在不同参数下的视觉差异。

下一步：进入阶段 6，实现机械臂语境动画，将滤波前后的关节信号映射到机械臂姿态和末端轨迹。
```
...
完成内容：新增 RobotArmWidget，绘制 2D 二连杆机械臂；将实时未滤波信号和滤波输出映射为关节角姿态；显示滤波前/滤波后两套机械臂；记录并绘制两条末端轨迹拖尾；主窗口可视化区域同时显示信号曲线和机械臂动画；重置时清空曲线与轨迹。

测试结果：Release 构建通过，生成 build/release/Filtering.exe；Debug 构建通过，生成 build-debug/debug/Filtering.exe。构建过程中仍出现 Qt license client 锁文件警告，但最终退出码为 0。

遗留问题：尚未打开 GUI 人工观察动画刷新稳定性、窗口缩放布局和轨迹显示效果。

下一步：完成构建验证后进入阶段 7，构造力/力矩传感器信号并对比基础平滑滤波器。
```

```text
日期：2026-05-28

本次目标：推进阶段 7 力/力矩传感器演示

完成内容：新增 ForceTorqueWidget，模拟接触突变、短时冲击和随机尖峰；内部同时运行滑动平均、EWMA、中值、Hampel 四种滤波器；在选择滑动平均、EWMA、中值、Hampel 时显示力/力矩对比图；说明区补充“响应速度 vs 平滑程度”的对比提示。

测试结果：Release 构建通过，生成 build/release/Filtering.exe；Debug 构建通过，生成 build-debug/debug/Filtering.exe。构建过程中仍出现 Qt license client 锁文件警告，但最终退出码为 0。

遗留问题：尚未打开 GUI 人工观察四种滤波器对尖峰和接触突变的视觉差异。

下一步：进入阶段 8，评估本机 OpenCV 路径并接入图像滤波演示。
```
